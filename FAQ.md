...
## exit code is '-11', not equals '0' 错误解决

**问题说明：**  
实验运行日志提示“exit code is '-11', not equals '0'”。

**常见原因：**  
1. **内存错误**（如越界访问、野指针、未分配/释放内存后操作等）。
2. **编译环境默认未启用 POSIX 功能**，导致如 `strdup()` 等标准库函数无法使用。

**解决办法：**

- **检查和修正内存相关错误：**  
  - 仔细检查指针使用、动态内存分配与释放过程，确保没有越界/非法访问；
  - 工具推荐：用 `valgrind` 检查内存问题，如运行：
    ```bash
    valgrind ./your_program
    ```
  - 常见错误如：访问已释放内存、未初始化指针、数组越界等。

- **启用 POSIX 功能支持（解决 `strdup` 相关问题）：**  
  如果编译器未默认启用 POSIX 标准，可以手动添加编译选项。例如，使用 GCC 时加上：
  ```bash
